  ...
  if (!presentationSuccess && !isHookedFailed) {
    ...
    if (sdl12Symbol) {
      ...
      if (sdl12_GetWMInfo(&wminfo) == 1) {
        ...
        GLIDE_LOG(INFO, "Vulkan",
                  "Extracted host X11 Window and Display from SDL 1.2. Window="
                      << m_nativeWindow << ", Display=" << m_nativeDisplay);
      }
    }
    ...
    if (!m_nativeDisplay) {
      ...
      GLIDE_LOG(INFO, "Vulkan",
                "Reusing host's active X11 Display connection for cooperative presentation.");
    }
