...
int main(int argc, char* argv[]) {
  std::cout << "[Game-SDL1.2-NoInject] Starting Snake game without window injection..." << std::endl;

  // 1. Initialize SDL Video
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    std::cerr << "[Game-SDL1.2-NoInject] SDL_Init failed: " << SDL_GetError() << std::endl;
    return 1;
  }
  ...
  if (!glideHandle) {
    std::cerr << "[Game-SDL1.2-NoInject] Failed to load libglide2x.so: " << dlerror() << std::endl;
    SDL_Quit();
    return 1;
  }
  ...
  if (!p_grGlideInit || !p_grSstSelect || !p_grSstWinOpen || !p_grBufferClear ||
    ...
    std::cerr << "[Game-SDL1.2-NoInject] Failed to resolve all Glide symbols!" << std::endl;
    dlclose(glideHandle);
    SDL_Quit();
    return 1;
  }
  ...
  if (!p_grSstWinOpen(0, GR_RESOLUTION_640x480, GR_REFRESH_60Hz,
    ...
    std::cerr << "[Game-SDL1.2-NoInject] grSstWinOpen failed!" << std::endl;
    p_grGlideShutdown();
    dlclose(glideHandle);
    SDL_Quit();
    return 1;
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        ...
        switch (event.key.keysym.sym) {
          ...
          case SDLK_q:
          case SDLK_ESCAPE:
            std::cout << "[Game-SDL1.2-NoInject] Exit key pressed, terminating..." << std::endl;
            running = false;
            break;
          default:
            break;
        }
        std::cout << "[Game-SDL1.2-NoInject] Received KEYDOWN: sym=" << event.key.keysym.sym
        ...
        std::cout << "[Game-SDL1.2-NoInject] Received KEYUP: sym=" << event.key.keysym.sym
                  << " | Direction Vector: (" << dx << ", " << dy << ")" << std::endl;
      }
    }
  ...
  std::cout << "[Game-SDL1.2-NoInject] Clean shutdown completed." << std::endl;
  return 0;
}
