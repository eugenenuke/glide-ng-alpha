...
int main(int argc, char* argv[]) {
  std::cout << "[Game-SDL2] Starting Snake game on SDL 2..." << std::endl;

  // 1. Initialize SDL 2
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    std::cerr << "[Game-SDL2] SDL_Init failed: " << SDL_GetError() << std::endl;
    return 1;
  }

  SDL_Window* window =
      SDL_CreateWindow("Snake Game (SDL 2)", SDL_WINDOWPOS_CENTERED,
                       SDL_WINDOWPOS_CENTERED, 640, 480, 0);
  if (!window) {
    std::cerr << "[Game-SDL2] SDL_CreateWindow failed: " << SDL_GetError()
              << std::endl;
    SDL_Quit();
    return 1;
  }
  ...
  if (SDL_GetWindowWMInfo(window, &wmInfo)) {
    if (wmInfo.subsystem == SDL_SYSWM_X11) {
      ...
      std::cout << "[Game-SDL2] Extracted X11 Window ID: " << winId
                << std::endl;
    } else {
      std::cerr << "[Game-SDL2] Not running on X11! Subsystem="
                << wmInfo.subsystem << std::endl;
      SDL_DestroyWindow(window);
      SDL_Quit();
      return 1;
    }
  } else {
    std::cerr << "[Game-SDL2] Failed to get WM Info!" << std::endl;
    SDL_DestroyWindow(window);
    SDL_Quit();
    return 1;
  }
  ...
  if (!glideHandle) {
    std::cerr << "[Game-SDL2] Failed to load libglide2x.so: " << dlerror()
              << std::endl;
    SDL_DestroyWindow(window);
    SDL_Quit();
    return 1;
  ...
  if (!p_grGlideInit || !p_grSstSelect || !p_grSstWinOpen || !p_grBufferClear ||
    ...
    std::cerr << "[Game-SDL2] Failed to resolve all Glide symbols!"
              << std::endl;
    dlclose(glideHandle);
    SDL_DestroyWindow(window);
    SDL_Quit();
  ...
  if (!p_grSstWinOpen(winId, GR_RESOLUTION_640x480, GR_REFRESH_60Hz,
    ...
    std::cerr << "[Game-SDL2] grSstWinOpen failed!" << std::endl;
    p_grGlideShutdown();
    dlclose(glideHandle);
    SDL_DestroyWindow(window);
    SDL_Quit();
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        ...
        switch (event.key.keysym.sym) {
          ...
          case SDLK_q:
          case SDLK_ESCAPE:
            std::cout << "[Game-SDL2] Exit key pressed, terminating..."
                      << std::endl;
            running = false;
            break;
          default:
        ...
        std::cout << "[Game-SDL2] Received KEYDOWN: sym="
        ...
        std::cout << "[Game-SDL2] Received KEYUP: sym=" << event.key.keysym.sym
  ...
  std::cout << "[Game-SDL2] Clean shutdown completed." << std::endl;
  return 0;
}
